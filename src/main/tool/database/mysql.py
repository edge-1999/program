...
class OperationMysqlData(object):
    ...
    def dow_data(self, db_name, tb_name, file_name):
        ...
        if file_name.lower().split('.')[-1] in MYSQL_WRITE_DATA_PD:
            print('文件格式正确，执行数据读取')
            # 读取mysql数据
            db = pd.read_sql(sql='select * from {}.{}'.format(db_name, tb_name), con=engine, index_col=False)
            print('数据读取成功，开始执行写入到文件->{}'.format(file_name))
            db.to_excel(file_name)  # 导出数据到excel
            print('写入成功！')
